...
using DuGu.XFLib.Effects;
using DuGu.XFLib.Extensions;
...
namespace DuGu.XFLib.Controls
{
	public class UniformGrid : Grid
	{
		...
		private static void ItemTapedCmdChanged(BindableObject bindable, object oldValue, object newValue)
		{
			...
			{
                //TapBinder.SetWithFeedback(v, false);
		...
		#region ItemWithFeedBack

		public static readonly BindableProperty ItemWithFeedBackProperty =
			BindableProperty.Create("ItemWithFeedBack",
                typeof(bool),typeof(UniformGrid),
                false,
				propertyChanged: ItemWithFeedBackPropertyChanged);
		...
		public bool ItemWithFeedBack
		{
			get { return (bool)GetValue(ItemWithFeedBackProperty); }
			set
			{
				SetValue(ItemWithFeedBackProperty, value);
			}
		}
		...
		{
            object source = this.ItemsSource;
            if(source is INotifyCollectionChanged)
            {
                var notify = source as INotifyCollectionChanged;
                notify.Wraper(add: (datas, idx) => this.Add(datas, idx),
